...
if [ $ACL_MODE == "DENY_SOME_ALLOW_ALL" ]; then
  ...
  for IP in ${IPS[@]}; do
    ...
    iptables -D $IPTABLES_DOCKER_CHAIN -i $DOCKER_CONTAINER_INTERFACE -m conntrack --ctorigdst $IP -j DROP
  done
  ...
  for IP in ${IPS[@]}; do
    ...
    iptables -D $IPTABLES_DOCKER_CHAIN -i $DOCKER_CONTAINER_INTERFACE -m conntrack --ctorigdst $IP -j ACCEPT
  done
else
...
# allow dns
iptables -D $IPTABLES_DOCKER_CHAIN -i $DOCKER_CONTAINER_INTERFACE -m conntrack --ctorigdst $WG_DEFAULT_DNS --ctorigdstport 53 -j ACCEPT
