...
if [ $MODE == "DENY_ALLOW" ]; then
  ...
  #   iptables -D FORWARD -o $OUT_INTERFACE -i $WG_INTERFACE -d $IP -j DROP
  # done
  iptables -D FORWARD -i $WG_INTERFACE -o $OUT_INTERFACE -j ACCEPT
  iptables -D FORWARD -o $OUT_INTERFACE -i $WG_INTERFACE -j ACCEPT
elif [ $MODE == "ALLOW_DENY" ]; then
  # for IP in ${IPS[@]}; do
  #   iptables -D FORWARD -i $WG_INTERFACE -o $OUT_INTERFACE -s $IP -j ACCEPT
  #   iptables -D FORWARD -o $OUT_INTERFACE -i $WG_INTERFACE -d $IP -j ACCEPT
  # done
  iptables -D FORWARD -i $WG_INTERFACE -o $OUT_INTERFACE -j DROP
  iptables -D FORWARD -o $OUT_INTERFACE -i $WG_INTERFACE -j DROP
else
